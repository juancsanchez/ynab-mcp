...
export function registerBudgetRecommendationTools(server: McpServer) {
  server.registerTool(
    ...
    async (args) => {
      try {
        ...
        const monthData: Array<{
          month: string;
          categories: Record<string, { name: string; group: string; activity: number; budgeted: number }>;
        }> = [];

        for (const month of monthsList) {
          const resp = await client.months.getBudgetMonth(args.budget_id, month);
          const cats: Record<string, { name: string; group: string; activity: number; budgeted: number }> = {};
        ...
        const catStats: Record<
          string,
          {
            ...
            monthly_budgeted: number[];
            avg_spending: number;
            avg_budgeted: number;
            overspent_months: number;
            underspent_months: number;
        ...
        for (const catId of allCatIds) {
          ...
          if (spending.length >= 2) {
            ...
            if (diff > firstAvg * 0.1) trend = "increasing";
            else if (diff < -firstAvg * 0.1) trend = "decreasing";
          }
        ...
        for (const [catId, stats] of Object.entries(catStats)) {
          if (stats.avg_spending === 0) continue;
