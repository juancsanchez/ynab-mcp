...
function toYnabSummary(t: TransactionDetail): YnabTransactionSummary {
  return {
    id: t.id,
    date: t.date,
...
function payeeSimilar(a: string, b: string): boolean {
...
Use this to identify missing transactions, duplicates, and discrepancies before approving.`,
    ...
    async (args) => {
      try {
        ...
        const bankEntries = args.bank_transactions as BankStatementEntry[];
        ...
        const ynabPool: TransactionDetail[] = resp.data.transactions.filter(
          (t) => !t.deleted
        );

        // Detect duplicates in bank statement
        const bankKeyCount: Record<string, number> = {};
        ...
        for (const entry of bankEntries) {
          ...
          const strictMatch = ynabPool.find(
            (t) =>
              !usedYnabIds.has(t.id) &&
              Math.abs(
                (new Date(t.date + "T00:00:00Z").getTime() - entryMs) /
                  (1000 * 60 * 60 * 24)
              ) <= dateTolerance &&
          ...
          const fuzzyMatch = ynabPool.find(
            (t) =>
              !usedYnabIds.has(t.id) &&
              Math.abs(
                (new Date(t.date + "T00:00:00Z").getTime() - entryMs) /
        ...
        const unmatchedYnab: YnabTransactionSummary[] = ynabPool
    ...
    async (args) => {
      try {
        ...
        const transactions = args.transactions.map((e) => ({
          ...
          memo: e.memo,
          cleared: markCleared ? ("cleared" as const) : ("uncleared" as const),
          approved: false,
        }));

        const resp = await client.transactions.createTransactions(
          args.budget_id,
          { transactions }
        );
