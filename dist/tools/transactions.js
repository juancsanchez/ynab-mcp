...
export function registerTransactionTools(server) {
    server.registerTool("get_transactions", {
        ...
        try {
            ...
            let serverKnowledge;
            if (args.account_id) {
                const resp = await client.transactions.getTransactionsByAccount(args.budget_id, args.account_id, args.since_date, args.type, args.last_knowledge_of_server);
                transactions = resp.data.transactions;
                serverKnowledge = resp.data.server_knowledge;
            }
            else {
                const resp = await client.transactions.getTransactions(args.budget_id, args.since_date, args.type, args.last_knowledge_of_server);
                transactions = resp.data.transactions;
                serverKnowledge = resp.data.server_knowledge;
            }
    ...
    server.registerTool("create_transaction", {
        ...
        try {
            ...
            const resp = await client.transactions.createTransaction(args.budget_id, { transaction });
            const t = resp.data.transaction;
            if (!t)
                return err("Transaction was not returned by API");
            return ok({
                id: t.id,
                date: t.date,
                amount: fromMilliunits(t.amount),
    ...
    server.registerTool("update_transaction", {
        ...
        try {
            ...
            const existing = await client.transactions.getTransactionById(args.budget_id, args.transaction_id);
            ...
            const transaction = {
                ...
                cleared: (args.cleared ?? ex.cleared),
                approved: args.approved ?? ex.approved,
                flag_color: (args.flag_color !== undefined
                    ? args.flag_color
                    : ex.flag_color),
            };
            const resp = await client.transactions.updateTransaction(args.budget_id, args.transaction_id, { transaction });
            const t = resp.data.transaction;
            return ok({
                id: t.id,
                date: t.date,
    ...
    server.registerTool("approve_transactions", {
        ...
        try {
            const client = getYnabClient();
            const transactions = args.transaction_ids.map((id) => ({
                id,
                approved: true,
            }));
            const resp = await client.transactions.updateTransactions(args.budget_id, { transactions });
            return ok({
    ...
    server.registerTool("flag_transaction", {
        ...
        try {
            ...
            const transaction = {
                ...
                date: ex.date,
                amount: ex.amount,
                flag_color: (args.flag_color ?? null),
                memo: args.memo ?? ex.memo ?? undefined,
            };
            const resp = await client.transactions.updateTransaction(args.budget_id, args.transaction_id, { transaction });
            const t = resp.data.transaction;
